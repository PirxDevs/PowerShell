...
if ([Environment]::Is64BitOperatingSystem) {
    $url = "https://download.mozilla.org/?product=firefox-msi-latest-ssl&os=win64&lang=$lang"
} else {
    $url = "https://download.mozilla.org/?product=firefox-msi-latest-ssl&os=win&lang=$lang"
}
...
if ($program) {
    Write-Log "Mozilla Firefox is already installed"
    return
}

# Download the MSI package
Write-Log "Downloading $url"
$msiPath = Join-Path $env:TEMP "mozilla_firefox.msi"
