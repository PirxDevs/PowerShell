...
#
# Google Chrome Installation Script for Automatic Deployment in AD via GPO.
...
# Check if the MSI package is already installed. If it is, exit the script.
$msiInstalled = Get-Package -ProviderName msi -Name "Google Chrome" -ErrorAction SilentlyContinue
if ($msiInstalled) {
...
$program = Get-Package -Provider Programs -Name "Google Chrome" -ErrorAction SilentlyContinue
if ($program) {
    Write-Log "Uninstalling non-MSI Google Chrome"
    $uninstallString = ([xml]$program.SwidTagText).SoftwareIdentity.Meta.UninstallString
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "$uninstallString", "--force-uninstall" -Wait
}
...
if ($enableLogs -eq $true) {
    $msiLogPath = Join-Path $logShare ($env:COMPUTERNAME + "_Goolge_Chrome_Install_$(Get-Date -Format 'yyyyMMdd_HHmmss').log")
